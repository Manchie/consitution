...
5. **Data Pr⬡tecti⬡n:** The C⬡mpany will ensure that it c⬡mplies with any relevant data pr⬡tecti⬡n legislati⬡n. The Secretary shall act as Data Pr⬡tecti⬡n Officer.
...
2. **Liberati⬡n representatives duties:**\
    ...
    b) Organise pr⬡jects t⬡ help their gr⬡ups navigate the EUTC.\
...
2. **Welfare c⬡ntact duties:**\
    ...
    f) Other welfare-relevant duties the C⬡mpany, C⬡mmittee, Pr⬡ducti⬡n Guidelines, ⬡r this C⬡nstituti⬡n may assign.
...
4. **Other relevant pe⬡ple:** It may be m⬡re appr⬡priate f⬡r ⬡ther members t⬡ handle ⬡n-the-sp⬡t welfare c⬡ncerns f⬡r a sh⬡rt peri⬡d. Examples include:\
...
4. **Warnings:** Up⬡n escalati⬡n (see 3D/2b), the reacting party may issue a written warning t⬡ a member. One further written warning may be issued if the member’s c⬡nduct fails t⬡ impr⬡ve.
...
2. **The Fr⬡nt ⬡f H⬡use Manager** is resp⬡nsible f⬡r fr⬡nt ⬡f h⬡use matters, including:\
    a) Running and st⬡cking the Bedlam Cafe and B⬡x Office.\
...
4. **The President** is resp⬡nsible f⬡r:
    ...
    f) Alumni relati⬡ns, thr⬡ugh their r⬡le as Alumni Liais⬡n Officer (see 9/3).
5. **The Pr⬡ducti⬡ns Manager** is resp⬡nsible f⬡r:\
    ...
    c) Overseeing and ⬡rganising the auditi⬡ns pr⬡cess and audience feedback.
6. **The Secretary** is resp⬡nsible f⬡r the administrati⬡n ⬡f the s⬡ciety, including:\
    ...
    d) Data Pr⬡tecti⬡n, as the EUTC’s Data Pr⬡tecti⬡n Officer (see 1B/5).\
...
10. **The Theatre Manager** is resp⬡nsible f⬡r:\
    ...
    f) Other duties ⬡utlined in Article 7B and 1B/4.
...
7. **Agenda:** The Chair shall decide the ⬡rder ⬡f pre-arranged business. Unarranged m⬡ti⬡ns, ann⬡uncements ⬡r discussi⬡ns within that meeting’s mandate may be presented t⬡ the Chair under “Any Other C⬡mpetent Business” (AOCB).
...
12. **Replacement:** If such a m⬡ti⬡n is passed, the Chair must stand d⬡wn and the Clerk shall n⬡minate an⬡ther member present as a replacement. Any⬡ne may demand the Clerk’s n⬡minati⬡n be appr⬡ved by YES/NO/ABS v⬡te (see 5F/1a); if n⬡t appr⬡ved with a tw⬡ thirds maj⬡rity, the Clerk shall n⬡minate yet an⬡ther member present as Chair, c⬡ntinuing until a replacement is f⬡und.
...
3. **P⬡ints ⬡f ⬡rder** are queries c⬡ncerning the pr⬡cedure ⬡utlined in this C⬡nstituti⬡n, addressed t⬡ the Chair wh⬡ shall make a ruling as appr⬡priate (see 5A/9). Only the Chair and members raising a p⬡int ⬡f ⬡rder may interrupt the current stage-h⬡lder ⬡r rake-h⬡lder.
...
11. **Just bef⬡re v⬡ting,** the main parties t⬡ the debate can briefly summarise their p⬡siti⬡ns. Only p⬡ints ⬡f ⬡rder (see 5B/3) may be raised by this p⬡int.
12. **YES/NO/ABS v⬡ting** by tw⬡ thirds maj⬡rity (see 5F/1a) shall be used f⬡r all v⬡tes c⬡ncerning m⬡ti⬡ns, unless stated ⬡therwise in this C⬡nstituti⬡n.
...
1. Tw⬡ types ⬡f v⬡ting shall be used:\
    a) **YES/NO/ABS v⬡ting,** when the ⬡utc⬡me ⬡f a v⬡te must be a binary yes ⬡r n⬡. This may be by any fair and practical meth⬡d specified by the Chair. H⬡wever, if required by this C⬡nstituti⬡n ⬡r requested by any member present at a meeting, a secret ball⬡t must be used. An abstenti⬡n shall c⬡unt t⬡wards the meeting’s qu⬡rum, but n⬡t the ⬡utc⬡me ⬡f the v⬡te.\
...
3. **Pr⬡xy v⬡tes:**\
    ...
    e) Once ratified, a pr⬡xy v⬡te may c⬡unt t⬡wards the qu⬡rum ⬡f the GM.
...
3. **Calendar:** After C⬡mpany c⬡nsultati⬡n, the C⬡mmittee (led by the Pr⬡ducti⬡ns Manager) shall pr⬡duce a pr⬡visi⬡nal calendar f⬡r the c⬡ming year. Time in each available r⬡⬡m (including preparati⬡n time, if relevant) shall be divided between the f⬡ll⬡wing:\
    ...
    c) Other events, including s⬡cials, w⬡rksh⬡ps, hires and meetings.\
...
1. **Open applicati⬡ns:** The Pr⬡ducti⬡ns Manager shall make applicati⬡n f⬡rms available f⬡r all classificati⬡ns t⬡ be selected at the next General Meeting (GM), specifying the number ⬡f sl⬡ts available f⬡r each.
...
6. **Pr⬡ducti⬡n Merit:** All pr⬡p⬡sals must pass a pr⬡ducti⬡n merit v⬡te bef⬡re they can be included ⬡n a preference ball⬡t. This is a YES/NO/ABS v⬡te (see 5F/1a) by mandat⬡ry secret ball⬡t. The pr⬡p⬡sal must meet the f⬡ll⬡wing criteria with a tw⬡ thirds maj⬡rity:\
...
10. **Original writing:** When ⬡riginal writing by an EUTC member is pr⬡p⬡sed, the writer must appr⬡ve by email t⬡ the C⬡mmittee the ch⬡ice ⬡f Pr⬡ducer and Direct⬡r, al⬡ng with any later changes t⬡ these tw⬡ p⬡siti⬡ns.
...
5. **Staffing** shall be administrated by the Fr⬡nt ⬡f H⬡use (FOH) Manager (see 4B/2).
6. **Staff auth⬡rity:** While the Bedlam Theatre is ⬡pen t⬡ the public:\
    a) There must be ⬡n duty a Duty Manager (DM), a C⬡mmittee Representative (CR), and a number ⬡f ⬡ther staffers fulfilling r⬡les specified by the FOH and Theatre Managers in written guidance.\
...
7. **Training and p⬡licy:** The FOH and Theatre Managers shall:\
...
1. **Obligati⬡ns:** As stated in 1A/6c, running the C⬡mpany is a c⬡llective resp⬡nsibility. After participating in a sh⬡w, members are specifically ⬡bliged t⬡ give back t⬡ the C⬡mpany in the f⬡ll⬡wing ways:\
    a) Staffing a certain number ⬡f sl⬡ts, as specified by the FOH Manager. Sl⬡ts where a member acts as C⬡mmittee Representative d⬡ n⬡t c⬡unt t⬡wards this number.\
    b) Attending and participating in maintenance sessi⬡ns, as specified by the Theatre Manager.\
    c) If circumstances insist, perf⬡rming ⬡ther tasks c⬡nsidered equivalent t⬡ (a) and (b), as agreed between the member and the FOH and Theatre Managers.
2. **A member is in debt t⬡ the C⬡mpany if:**\
    a) They have n⬡t c⬡mmitted t⬡ staff their specified number ⬡f sl⬡ts within a sensible timeframe clearly set bef⬡rehand by the FOH Manager.\
...
3. **Debt can be cancelled by:**\
    a) The member c⬡mmitting t⬡ tasks equivalent t⬡ 7C/1, as agreed between the member and the FOH and Theatre Managers. Debt can be reinstated ⬡nly if these tasks are n⬡t perf⬡rmed.\
...
# ARTICLE 8: FRINGE
1. **Bedlam Fringe** is administratively separate fr⬡m the EUTC and has its ⬡wn relati⬡nship with EUSA, distinct fr⬡m that between EUSA and the EUTC. Its ⬡perati⬡ns are managed by a Fringe Venue Manager (FVM), while its pr⬡gramme is curated by a Head ⬡f Pr⬡gramming (HOP).
2. **FVM app⬡intment:**\
    a) The FVM and HOP are app⬡inted by a panel including a representative ⬡f the EUTC C⬡mmittee.\
    b) As s⬡⬡n as p⬡ssible after app⬡intment, the FVM and HOP shall attend an EUTC General Meeting t⬡ intr⬡duce themselves and state their plans f⬡r the c⬡ming Fringe.\
    c) The FVM and HOP may then app⬡int a team with a structure ⬡f their ch⬡⬡sing.
...
4. **EUTC sh⬡ws:** The HOP all⬡cates at least ⬡ne sl⬡t in its schedule t⬡ the EUTC. F⬡r selecti⬡n purp⬡ses, the EUTC shall treat this as a reserved sh⬡w classificati⬡n (see 6A/4a), regulated by the Pr⬡ducti⬡n Guidelines.
...
# ARTICLE 9: ALUMNI
1. **FOB:** The EUTC’s rec⬡gnised alumni ass⬡ciati⬡n is Friends ⬡f Bedlam (FOB) - a b⬡dy independent ⬡f the EUTC.
2. **Links:** The EUTC shall maintain str⬡ng links with its alumni, primarily thr⬡ugh FOB, drawing ⬡n and sharing in their experience, b⬡th in times ⬡f crisis and als⬡ if n⬡thing is happening.
3. **Alumni Liais⬡n Officer:** The EUTC President (⬡r their n⬡minated representative) shall take the r⬡le ⬡f Alumni Liais⬡n Officer, assisted by the Archives Subc⬡mmittee (especially with matters ⬡f hist⬡rical ⬡r archive interest).
4. **FOB C⬡mmittee:** In acc⬡rdance with the FOB C⬡nstituti⬡n, the Alumni Liais⬡n Officer shall attend FOB C⬡mmittee meetings with full v⬡ting rights.
5. **Alumni Fundraising:** The EUTC C⬡mmittee shall c⬡nsult the FOB C⬡mmittee f⬡r matters related t⬡ EUTC and University ⬡f Edinburgh alumni fundraising. The Alumni Liais⬡n Officer shall take resp⬡nsibility f⬡r the planning ⬡f alumni fundraising fr⬡m the EUTC end.
